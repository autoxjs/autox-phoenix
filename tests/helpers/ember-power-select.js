...
const TriggerRegex = /ember-power-select-trigger(.*ember\d+)/;
export default function() {
  const isEmberOne = Ember.VERSION.match(/1\.13/);

  Ember.Test.registerAsyncHelper('selectChoose', function(app, cssPath, value) {
    const id = find(cssPath).find('.ember-power-select-trigger').attr('id').match(TriggerRegex)[1];
    // If the dropdown is closed, open it
    if (Ember.$(`.ember-power-select-dropdown${id}`).length === 0) {
    ...
    andThen(function() {
      let potentialTargets = $(`.ember-power-select-dropdown${id} .ember-power-select-option:contains("${value}")`).toArray();
      let target;
  ...
  Ember.Test.registerAsyncHelper('selectSearch', function(app, cssPath, value) {
    ...
    let dropdownIsClosed = Ember.$(`.ember-power-select-dropdown${id}`).length === 0;
    if (dropdownIsClosed) {
      nativeMouseDown(`${cssPath} .ember-power-select-trigger`);
      wait();
    }
    ...
    if (isMultipleSelect) {
      ...
      if (isEmberOne) {
        triggerEvent(`.ember-power-select-dropdown${id} .ember-power-select-search input`, 'input');
      }
    }
  ...
  Ember.Test.registerAsyncHelper('clearSelected', function(app, cssPath, value) {
    ...
    try {
      ...
      console.warn('css path to clear btn not found ' + value);
      throw e;
    }
  });
}